...
Provides a global permission policy for the main agent and dependent extensions. Open the mode selector with `Ctrl+Alt+M` (or `/permissions`); it offers `manual`, `accept-edits`, `auto`, `read-only`, optional `plan`, and `dangerous`. `manual` confirms writes, edits, shell commands, and unknown tools. `accept-edits` automatically accepts direct `write`/`edit` operations only inside the working directory and never auto-approves sensitive `.git`, `.env`, or `.ssh` targets; it still confirms out-of-scope edits, shell commands, concurrent workflow mutations, and unknown tools. `auto` first applies the same deterministic safety barriers, then may send an otherwise uncertain shell command—not conversation history—to an authenticated explicitly-free model or one estimated to cost at most $0.001 per classification. The isolated classifier has no tools and can only return `ALLOW` or `ASK`; malformed output, timeout, unavailable models, obvious mutation/network/install commands, and all uncertainty fall back to confirmation. Use `/permissions classifier status`, `/permissions classifier on`, or `/permissions classifier off` to inspect or persistently control it. Decisions are cached only in memory for the session. `read-only` blocks mutations. `plan` is registered by the separate plan-mode extension. `dangerous` bypasses every Pi++ tool confirmation, requires an explicit warning confirmation, is visibly marked in the footer, and deliberately resets to `manual` after restart rather than persisting. Confirmation-required operations fail closed without a TUI. Persistent mode and classifier settings are stored in `~/.pi/agent/piplusplus-permissions.json`.
