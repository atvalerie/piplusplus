...
| `maxTurns` | Subagents/print mode support a maximum agentic-turn count. | User-owned off/custom/model modes default to unlimited, can impose one limit on every worker, or accept orchestrator per-agent limits; parent NDJSON tracking terminates a limited tool-using worker exactly once. | Pi++ user-control extension |
| Cost/turn limits | Claude subagents expose `maxTurns`; SDK loops can expose a USD budget, but public Claude Code workflows have no aggregate token-budget field. | Independent user-owned off/custom/model modes control aggregate agent/token/cost scheduling thresholds and worker turns; both default off. The deadline remains active. | Pi++ extension, off by default |
