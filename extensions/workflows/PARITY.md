...
| Cost/turn limits | Print mode exposes turn and USD budget flags. | Workflow-wide agent/token/cost/deadline budgets plus partial results and `budget_exhausted`. | Pi++ extension |
| Size guidance | The target contract uses small/medium/large/unrestricted advisory sizes. | Same thresholds shape generation; hard runtime budgets remain authoritative. | Match |
