...
const reviewStatuses = [codeReview.status, securityReview.status];
...
if (!await approve("Implementation plan", JSON.stringify(plan, null, 2))) return "Implementation canceled at the plan approval gate.";
const parsedPlan = plan;
if (parsedPlan.status !== "completed") return "Planning did not produce an implementable plan: " + JSON.stringify(plan);
...
const implementation = await agent("Implement the approved plan for the original objective.\\n\\nApproved plan:\\n" + JSON.stringify(plan), { id: "implementer", label: "Implementer", profile: "implementer", writePaths: parsedPlan.targetFiles });
if (implementation.status !== "completed") return "Implementation escalated or blocked: " + JSON.stringify(implementation);
phase("Quality checks");
const quality = await agent("Run repository-defined checks and correct only in-scope quality failures. Do not weaken tests.\\n\\nApproved plan:\\n" + JSON.stringify(plan) + "\\n\\nImplementation report:\\n" + JSON.stringify(implementation), { id: "quality-fixer", label: "Quality fixer", profile: "quality-fixer", writePaths: parsedPlan.targetFiles });
phase("Independent verification");
const [codeReview, securityReview] = await parallel([
  () => agent("Review the implementation against the objective and approved plan.\\n\\nPlan:\\n" + JSON.stringify(plan) + "\\n\\nImplementation report:\\n" + JSON.stringify(implementation), { id: "code-review", label: "Code reviewer", profile: "reviewer" }),
  () => agent("Security-review the implementation and changed boundaries.\\n\\nPlan:\\n" + JSON.stringify(plan) + "\\n\\nImplementation report:\\n" + JSON.stringify(implementation), { id: "security-review", label: "Security reviewer", profile: "security-reviewer" })
]);
phase("Synthesis");
...
const verificationStatuses = [quality.status, codeReview.status, securityReview.status];
