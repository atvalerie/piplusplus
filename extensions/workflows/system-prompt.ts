...
export function buildWorkflowSystemInstructions(options: {
	...
	return [
		...
		"Declare size (small <5, medium <15, large <50, unrestricted) and hard budgets when appropriate. maxAgents/maxTokens/maxCost stop new workers and preserve partial results; maxTurns bounds one worker. Avoid overlapping parallel edits. writePaths bounds direct edits, but unconfined shell/custom mutations require explicit user acknowledgement.",
